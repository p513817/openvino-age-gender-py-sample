...
class DiscountInfo():
    def __init__(self) -> None:
        ...
        self.define_mapping_table()
        self.generate_palette()
        
    def define_mapping_table(self):
        
        # Define information
        self.mapping_table = {
            "male":{
                "50+"   : "20% Off for Rolex Watches",
                "30-50" : "30% Off for Trendy males",
                "30-"   : "20% Off for Video Game",
            },
            "female":{
                "50+"   : "25% Off for Jewelry",
                "30-50" : "30% Off for Boutique package",
                "30-"   : "20% Off for Wonmen's Clothes",
            }
        }
    ...
    def __call__(self, age, gender):
        ...
        """
        return self.mapping_table[gender.lower()][self.formulation(age)]
...
if __name__ == '__main__':
    ...
    app = DiscountInfo() if args.app else None
    ...
    while True:
        ...
        for i in range(nets[PRI][RESULT][out_blob].shape[2]):
            ...
            if conf > thres:
                ...
                baseLine = cv2.getTextSize(dets_text, FONT_FACE, FONT_SCALE, FONT_THICK)[1]
                text_x, text_y = x1, (y1 - baseLine)

                # Draw Face Bounding Box
                cv2.rectangle(org_image, (x1, y1), (x2, y2), trg_color, 2)

                cv2.putText(org_image, dets_text, (text_x, text_y), FONT_FACE, FONT_SCALE, (0, 0, 0), FONT_THICK+1, cv2.LINE_AA)
                cv2.putText(org_image, dets_text, (text_x, text_y), FONT_FACE, FONT_SCALE, trg_color, FONT_THICK, cv2.LINE_AA)

        # Resize Whole Image
        org_image = cv2.resize(org_image, (nets[ORG][WIDTH], nets[ORG][HEIGHT]))
        
        # Draw information
        org_image, end_pos = draw_device_info(org_image, ie, nets[SEC], None, "SECONDARY")
        org_image, end_pos = draw_device_info(org_image, ie, nets[PRI], end_pos, "PRIMARY")

        # Draw FPS
        t_end = time.time()
        fps = round((1/(t_end - t_start)), 3)
        cv2.putText(org_image, "FPS: {}".format(fps), (20, 20), FONT_FACE, FONT_SCALE, (0, 0, 0), FONT_THICK+1, cv2.LINE_AA)
        cv2.putText(org_image, "FPS: {}".format(fps), (20, 20), FONT_FACE, FONT_SCALE, (0, 0, 255), FONT_THICK, cv2.LINE_AA)
        ...
        logging.warning(ie.get_metric(metric_name="DEVICE_THERMAL", device_name="MYRIAD"))
